...
def main():
	...
	parser.add_argument("-d", "--draft-mode", action = "store_true", help = "Produce document as quickly as possible, e.g., by skipping two-pass rendering. This may lead to broken cross-references in the final file, but will render more quickly. It also will only render the solution (i.e., implies --only-solution).")
	...
	parser.add_argument("-r", "--randomize-task-seeds", action = "store_true", help = "Where no explicit task seed is set, randomize it.")
	...
	renderer = ExamRenderer(args.definition_json, output_source_doc = args.output_source_doc, draft_mode = args.draft_mode, randomize_task_seeds = args.randomize_task_seeds, verbose = args.verbose)
	while True:
		if (not args.only_solution) and (not args.draft_mode):
